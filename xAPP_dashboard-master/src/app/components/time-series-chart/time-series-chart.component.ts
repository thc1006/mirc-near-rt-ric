...
import { 
  ...
  ChartClickEventParams,
  DataZoomEventParams,
  EchartsOption
} from '../../services/oran-types';
...
export class TimeSeriesChartComponent implements OnInit, OnDestroy {
  ...
  public chartType: 'line' | 'area' | 'scatter' = 'line';
  public timeRange: '1h' | '6h' | '24h' | '7d' = '1h';
  public refreshInterval: 5 | 10 | 30 | 60 = 5; // seconds
  public showAnomalies = true;
  public showPredictions = false;
  public autoRefresh = true;

  public metrics: ChartMetric[] = [
    {
      id: 'e2-latency',
  ...
  public selectedMetrics: string[] = ['e2-latency', 'cpu-usage', 'memory-usage'];
  ...
  constructor(
    private readonly yangDataService: YangDataService,
  ...
  public toggleMetric(metricId: string): void {
  ...
  public setChartType(type: 'line' | 'area' | 'scatter'): void {
    this.chartType = type;
    this.updateChartOptions();
  }

  public setTimeRange(range: '1h' | '6h' | '24h' | '7d'): void {
  ...
  private initializeChart(): void {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      this.updateChartOptions();
      this.chart.on('click', (params: ChartClickEventParams) => this.onChartClick(params));
      this.chart.on('datazoom', (params: DataZoomEventParams) => this.onDataZoom(params));
    });
  }
  ...
  private updateChartOptions(): void {
    ...
    const option: EchartsOption = {
      ...
      tooltip: {
        ...
        formatter: (params: TooltipFormatterParams[] | TooltipFormatterParams): string => {
          ...
          params.forEach((param: TooltipFormatterParams) => {
  ...
  private getAnomalyPoints(metric: ChartMetric): echarts.MarkPointComponentOption {
    ...
    if (!this.showAnomalies || anomalies.length === 0) {
      return {};
    }
  ...
  private setupResizeObserver(): void {
    if (typeof window === 'undefined' || !('ResizeObserver' in window) || !this.chartContainer) {
      return;
    }
  ...
  private onChartClick(params: ChartClickEventParams): void {
  ...
  private onDataZoom(params: DataZoomEventParams): void {
